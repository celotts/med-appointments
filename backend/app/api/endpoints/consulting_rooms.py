from core.crud_consulting_room import (
...
async def list_consulting_rooms(
    ...
    return await get_consulting_rooms(db, skip, limit)
...
async def get_consulting_room(room_id: str, db: AsyncSession = Depends(get_db)):
    room = await get_consulting_room(db, room_id)
    if not room:
...
async def create_consulting_room(
    ...
    return await create_consulting_room(db, room)
...
async def update_consulting_room(
    ...
    db_room = await update_consulting_room(db, room_id, room)
    if not db_room:
        raise HTTPException(status_code=404, detail="Consulting room not found")
    return db_room
...
async def delete_consulting_room(room_id: str, db: AsyncSession = Depends(get_db)):
    db_room = await delete_consulting_room(db, room_id)
    if not db_room:
        raise HTTPException(status_code=404, detail="Consulting room not found")
    return None