...
router = APIRouter(prefix="/medasist", tags=["Medasist IA - Scheduling Assistant"])


# ─── Helper: find free slots for a doctor ────────────────────────────────
...
async def suggest_reschedule(
    ...
    """
    Given an appointment and preferred new date, suggest the best alternative slots.
    """
    ...
    # Score slots: prefer morning, closer to original time
    original_hour = appt.start_datetime.hour
    scored = []
    for slot in free_slots:
        slot_hour = datetime.fromisoformat(slot.start).hour
        # Score: closer to original hour = higher score, morning bonus
...
async def suggest_followup(
    ...
    """
    Suggest the next follow-up date based on visit type.
    """
    ...
    # Check if recommended date falls on weekend, push to Monday
